...
subroutine output(dir, iBlock, iOutputType)
  ...
  case ('3DALL')
     ...
     nvars_to_write = 13+nSpeciesTotal+nSpecies+nIons &
          ...
          +9 & ! (neutral: 1. ion-drag, 2. viscosity, 3. gwaccel
          +3 & ! (Coriolis related terms)
          +3 & ! (3 Dim PresGrad variable)
          +2 ! (Gedy_pot & Gedy_fac)
  ...
  case ('1DALL')
     ...
     ! added ion-drag, viscosity & gwaccel, yuhong, 11/20/2024
     ! added Coriolis Forces, 3-Dim, yuhong, 11/25/2024
     ! added PresGrad Forces, 3-Dim, yuhong, 01/10/2025
     ! added Gedy_pot & Gedy_fac
     nvars_to_write = 13+nSpeciesTotal+nSpecies+nIons+3+2+1+9+3+3+2 !nSpecies+5
  ...
  subroutine output_header
    ...
    if (cType(3:5) == "ALL" .or. cType(3:5) == "ION") then
       ...
       if (cType(3:5) == "ALL") then
          ...
          ! add NW terms: ion-drag, viscosity & gwaccel
          write(iOutputUnit_,"(I7,A1,a)") iOff+1, " ", "ion_drag. E"
          write(iOutputUnit_,"(I7,A1,a)") iOff+2, " ", "ion_drag. N"
          write(iOutputUnit_,"(I7,A1,a)") iOff+3, " ", "ion_drag. Z"
          
          write(iOutputUnit_,"(I7,A1,a)") iOff+4, " ", "viscosity. E"
          write(iOutputUnit_,"(I7,A1,a)") iOff+5, " ", "viscosity. N"
          write(iOutputUnit_,"(I7,A1,a)") iOff+6, " ", "viscosity. Z"
          
          write(iOutputUnit_,"(I7,A1,a)") iOff+7, " ", "gwa. E"
          write(iOutputUnit_,"(I7,A1,a)") iOff+8, " ", "gwa. N"
          write(iOutputUnit_,"(I7,A1,a)") iOff+9, " ", "gwa. Z"

          ! yuhong, 11/25/2024
          write(iOutputUnit_,"(I7,A1,a)") iOff+1, " ", "Coriolis. East"
          write(iOutputUnit_,"(I7,A1,a)") iOff+2, " ", "Coriolis. North"
          write(iOutputUnit_,"(I7,A1,a)") iOff+3, " ", "Coriolis. Vertical"

          ! yuhong, 01/10/2025
          write(iOutputUnit_,"(I7,A1,a)") iOff+1, " ", "PresGrad. East"
          write(iOutputUnit_,"(I7,A1,a)") iOff+2, " ", "PresGrad. North"
          write(iOutputUnit_,"(I7,A1,a)") iOff+3, " ", "PresGrad. Vertical"
          
          ! add FAC outputs for # Satellite, yuhong, 10/01/2021
          write(iOutputUnit_,"(I7,A1,a)") iOff+1, " ", "Gedy_fac"
          write(iOutputUnit_,"(I7,A1,a)") iOff+2, " ", "Gedy_fac1"
          ...
          !---------------------------------------------------------------    
          !---------------------------------------------------------------                           
          !! add neutral wind force: 1. ion-drag, 2. viscosity 3. awg                                

          write(iOutputUnit_,"(I7,A1,a)") iOff+1, " ", "ion_drag. East"
          write(iOutputUnit_,"(I7,A1,a)") iOff+2, " ", "ion_drag. North"
          write(iOutputUnit_,"(I7,A1,a)") iOff+3, " ", "ion_drag. Vertical"
          
          write(iOutputUnit_,"(I7,A1,a)") iOff+4, " ", "viscosity. East"
          write(iOutputUnit_,"(I7,A1,a)") iOff+5, " ", "viscosity. North"
          write(iOutputUnit_,"(I7,A1,a)") iOff+6, " ", "viscosity. Vertical"

          write(iOutputUnit_,"(I7,A1,a)") iOff+7, " ", "awg. East"
          write(iOutputUnit_,"(I7,A1,a)") iOff+8, " ", "awg. North"
          write(iOutputUnit_,"(I7,A1,a)") iOff+9, " ", "awg. Vertical"
...
subroutine output_3dall(iBlock)
  ...
  use ModSources, only: GWAccel ! yuhong, 11/19/2024
  !use ModVertical, only: Coriolis, CoriolisSin, CoriolisCos ! yuhong, 11/25/2024
     ...
     do iLat=-1,nLats+2
        ...
        do iLon=-1,nLons+2
           ...
           write(iOutputUnit_)       &
                ...
                ! yuhong, 11/20/2024
                (IonDrag(iiLon,iiLat,iiAlt,i),i=1,3), &
                (Viscosity(iiLon,iiLat,iiAlt,i),i=1,3), &
                (GWAccel(iiLon,iiLat,iiAlt,i),i=1,3), &
                ! yuhong, 11/25/2024
                (CoriolisF4(iiLon,iiLat,iiAlt,i),i=1,3), &
                ! yuhong, 01/10/2025
                (PresGrad(iiLon,iiLat,iiAlt,i),i=1,3), &
                ! qingyu, 11/24/2020
...
subroutine output_1dall(iiLon, iiLat, iBlock, rLon, rLat, iUnit)
  ...
  ! yuhong, 11/20/2024, add ion-drag, viscosity & gwa
  ! yuhong, 11/25/2024, add coriolis 3-dim
  integer, parameter :: nVars = 13+nSpeciesTotal+nSpecies+nIons+3+2+1+9+3+3+3 !nSpecies+5
...
!     enddo
        ...
        ! yuhong, 11/20/2024
        iOff = iOff + 1
        Tmp = IonDrag(0:nLons+1,0:nLats+1,iAlt,iEast_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)

        iOff = iOff + 1
        Tmp = IonDrag(0:nLons+1,0:nLats+1,iAlt,iNorth_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)

        iOff = iOff + 1
        Tmp = IonDrag(0:nLons+1,0:nLats+1,iAlt,iUp_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)

        !!------------------
        iOff = iOff + 1
        Tmp = Viscosity(0:nLons+1,0:nLats+1,iAlt,iEast_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)

        iOff = iOff + 1
        Tmp = Viscosity(0:nLons+1,0:nLats+1,iAlt,iNorth_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)

        iOff = iOff + 1
        Tmp = Viscosity(0:nLons+1,0:nLats+1,iAlt,iUp_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)        

        !!-------------------
        iOff = iOff + 1
        Tmp = GWAccel(0:nLons+1,0:nLats+1,iAlt,iEast_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)

        iOff = iOff + 1
        Tmp = GWAccel(0:nLons+1,0:nLats+1,iAlt,iNorth_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)

        iOff = iOff + 1
        Tmp = GWAccel(0:nLons+1,0:nLats+1,iAlt,iUp_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)
        
        ! yuhong, 11/25/2024  CoriolisF4
        iOff = iOff + 1
        Tmp = CoriolisF4(0:nLons+1,0:nLats+1,iAlt,iEast_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)
        
        iOff = iOff + 1
        Tmp = CoriolisF4(0:nLons+1,0:nLats+1,iAlt,iNorth_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)
        
        iOff = iOff + 1
        Tmp = CoriolisF4(0:nLons+1,0:nLats+1,iAlt,iUp_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)

        ! yuhong, 01/10/2025
        iOff = iOff + 1
        Tmp = PresGrad(0:nLons+1,0:nLats+1,iAlt,iEast_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)

        iOff = iOff + 1
        Tmp = PresGrad(0:nLons+1,0:nLats+1,iAlt,iNorth_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)

        iOff = iOff + 1
        Tmp = PresGrad(0:nLons+1,0:nLats+1,iAlt,iUp_)
        Vars(iOff) = inter(Tmp,iiLon,iiLat,rlon,rlat)
